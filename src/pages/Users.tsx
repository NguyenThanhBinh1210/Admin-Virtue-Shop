...
const Users = () => {
  ...
  return (
    <div className='p-5'>
      ...
      <div className='relative rounded-md overflow-x-auto shadow-md sm:rounded-lg'>
        <table className='w-full text-sm text-left text-gray-500 dark:text-gray-400'>
          <thead className='text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400'>
            <tr>
              <th scope='col' className='px-6 py-3 tablet:hidden mobile:hidden'>
                Id
              </th>
          ...
          {userDataNotAdmin && (
            <tbody>
              {userDataNotAdmin.map((item: any) => (
                ...
                >
                  <td className='px-6 py-4 tablet:hidden mobile:hidden'>{item._id}</td>
                  ...
                  <td className='px-6 py-4 flex gap-x-[10px]'>
                    <Link to={`/user-detail/${item._id}`} className=''>
                      <button className='relative inline-flex items-center justify-center p-0.5 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-cyan-500 to-blue-500 group-hover:from-cyan-500 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none focus:ring-cyan-200 dark:focus:ring-cyan-800"'>
                        <span className='relative px-5 tablet:text-[12px] transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0'>
                          Thông tin
                        </span>
                      </button>
                    </Link>{' '}
                    ...
                    >
                      <span className='relative px-5 h-full flex items-center tablet:text-[12px] transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0'>
                        Xoá
                      </span>
                    </button>
                  </td>
      ...
      {isLoading && (
        <div role='status' className='mx-auto flex justify-center mt-2'>
          <svg
            aria-hidden='true'
